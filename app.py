...
# --- THE NEW SYNC STRATEGY ---
def save_game_state(data):
    try:
        # We use a fixed name. Cloudinary will assign a new version number 
        # instead of 'overwriting', which avoids the security error.
...
def load_game_state():
    try:
        # We fetch the master file. By adding a timestamp (?t=), we force 
        # the phone to bypass its cache and see the newest version.
        t = int(time.time())
        url = f"https://res.cloudinary.com/{st.secrets['CLOUDINARY_CLOUD_NAME']}/raw/upload/game_state_master.json?t={t}"
...
# Load current state into the app
...
# --- THE REFRESH BUTTON (Crucial for the other phones) ---
